    ...
    def inferCam(self, raw_image):
        ...
        for i in range(self.batch_size):
            ...
            for j in range(len(result_boxes)):
                ...
                num_object += 1
        return batch_image_raw, end - start, num_object
...
class inferThreadCam(threading.Thread):
    def __init__(self, yolov5_wrapper, video_file:str=None):
        threading.Thread.__init__(self)
    ...
    def run(self):
        control = np.zeros((100, 500, 3), np.uint8)
        cv2.namedWindow('control')
        cv2.createTrackbar('IOU', 'control', 1, 99, change_IOU)
        cv2.createTrackbar('CONF', 'control', 1, 99, change_conf)
        while True:
            ret, frame = self.cap.read()
            if ret:
                cv2.imshow('control', control)
                cv2.imshow("frame", frame)
                batch_image_raw, use_time, num_object = self.yolov5_wrapper.inferCam(frame)
                for output_img in batch_image_raw:
                    cv2.imshow("Detected", output_img)
                print("time->{:.2f}ms, FPS->{:.2f}, num_object->{}".format(use_time * 1000, 1 / use_time, num_object))
                print(f"CONF_THRESH={CONF_THRESH}, IOU_THRESHOLD={IOU_THRESHOLD}")
                if cv2.waitKey(1) & 0xFF == ord("q"):
                    break
            else:
                break
        print("inferThreadCam ends.")
        self.cap.release()
...
class warmUpThread(threading.Thread):
    ...
    def run(self):
        ...
        print(
            "warm_up->{}, time->{:.2f}ms".format(
                batch_image_raw[0].shape, use_time * 1000
            )
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Destroying yolov5_wrapper...")
